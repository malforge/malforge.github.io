...
|Member|Description|
|---|---|
|[IsAndModeEnabled](Sandbox.ModAPI.Ingame.IMyEventControllerBlock@IsAndModeEnabled)|Returns whether and mode is enabled. When enabled all selected blocks have to pass the test to trigger the action.|
|[IsLowerOrEqualCondition](Sandbox.ModAPI.Ingame.IMyEventControllerBlock@IsLowerOrEqualCondition)|Gets/sets if the condition is lower or equal, when false condition is Higher or equal.|
|[Threshold](Sandbox.ModAPI.Ingame.IMyEventControllerBlock@Threshold)|Gets/sets Trigger threshold, which is used to compare if the event is above or bellow|
...
**Type Definitions:**  
* MyObjectBuilder_EventControllerBlock/EventControllerLarge  
* MyObjectBuilder_EventControllerBlock/EventControllerSmall
