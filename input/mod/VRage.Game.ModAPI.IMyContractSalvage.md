...
```csharp
public interface IMyContractSalvage: IMyContract
```

Describes salvage space contract where player must retrieve a datapad from a damaged grid

## Properties

|Member|Description|
|---|---|
|[GridPosition](VRage.Game.ModAPI.IMyContractSalvage@GridPosition)|Gets the position where the salvage grid is spawned|
|[PrefabName](VRage.Game.ModAPI.IMyContractSalvage@PrefabName)|Gets the prefab name used for the salvage grid|
|[TargetGridId](VRage.Game.ModAPI.IMyContractSalvage@TargetGridId)|Gets the entity ID of the target grid containing the objective|
