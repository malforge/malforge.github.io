...
```csharp
public interface IMyContractPvEBounty: IMyContract
```

Describes PvE bounty contract that player can take in contract block (mods interface). Player should kill a spawned NPC target.

## Properties

|Member|Description|
|---|---|
|[TargetFactionId](VRage.Game.ModAPI.IMyContractPvEBounty@TargetFactionId)|The faction ID of the target NPC to hunt.|
|[TargetPosition](VRage.Game.ModAPI.IMyContractPvEBounty@TargetPosition)|The position where the target will spawn.|
|[TargetSpawnGroup](VRage.Game.ModAPI.IMyContractPvEBounty@TargetSpawnGroup)|The spawn group used to spawn the NPC target.|
