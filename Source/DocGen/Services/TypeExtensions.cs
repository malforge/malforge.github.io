...
namespace DocGen.Services
{
    internal static class TypeExtensions
    {
        ...
        /// <summary>
        ///     Gets a formatted full name for a member suitable for file names.
        ///     - Nested types are separated by +
        ///     - Generics are designated without their backtick and number, but in the form of { T1, T2 } instead
        ///     - Members use @ as a separator
        /// </summary>
        public static string GetFileNameFormattedFullName(this MemberInfo memberInfo)
        {
            switch (memberInfo)
            {
                case Type type:
                    return GetTypeFileNameFormattedFullName(type);
                case ConstructorInfo constructorInfo:
                    return $"{GetTypeFileNameFormattedFullName(constructorInfo.DeclaringType)}@{memberInfo.Name}";
                case FieldInfo fieldInfo:
                    return $"{GetTypeFileNameFormattedFullName(fieldInfo.DeclaringType)}@{fieldInfo.Name}";
                case PropertyInfo propertyInfo:
                    return $"{GetTypeFileNameFormattedFullName(propertyInfo.DeclaringType)}@{propertyInfo.Name}";
                case EventInfo eventInfo:
                    return $"{GetTypeFileNameFormattedFullName(eventInfo.DeclaringType)}@{eventInfo.Name}";
                case MethodInfo methodInfo:
                    return $"{GetTypeFileNameFormattedFullName(methodInfo.DeclaringType)}@{GetMethodNameForFileName(methodInfo)}";
            }

            return null;
        }

        static string GetTypeFileNameFormattedFullName(Type type)
        {
            if (type == null)
                return null;
            ...
            // Build the full path with nested types
            var typeHierarchy = new List<string>();
            var currentType = type;
            while (currentType != null)
            {
                typeHierarchy.Insert(0, GetTypeNameForFileName(currentType));
                currentType = currentType.DeclaringType;
            }

            var fullTypeName = string.Join("+", typeHierarchy);

            if (string.IsNullOrEmpty(namespaceName))
                return fullTypeName;

            return $"{namespaceName}.{fullTypeName}";
        }

        static string GetTypeNameForFileName(Type type)
        {
            if (type.IsGenericType || type.IsGenericTypeDefinition)
            {
                var name = type.Name;
                ...
                // Filter out generic arguments from parent types if this is a nested type
                if (type.DeclaringType != null && type.DeclaringType.IsGenericType)
                {
                    var parentArgCount = type.DeclaringType.GetGenericArguments().Length;
                    genericArgs = genericArgs.Skip(parentArgCount).ToArray();
                }

                if (genericArgs.Length > 0)
                {
                    var genericNames = genericArgs.Select(arg => arg.Name);
        ...
        static string GetMethodNameForFileName(MethodInfo method)
        {
