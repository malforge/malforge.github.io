...
namespace DocGen.Services
{
    internal class ApiEntry
    {
        ...
        ApiEntry(ProgrammableBlockApi api, MemberInfo member, string assemblyName, string namespaceName, string name, string xmlDocKey, bool isWhitelisted)
        {
            ...
            SuggestedFileName = ToSafeFileName(member.GetFileNameFormattedFullName());
        }
        ...
        {
            return ToString(ApiEntryStringFlags.Default);
        }
        ...
        string ToParameterString(ParameterInfo parameterInfo, int index, ApiEntryStringFlags flags)
        {
            ...
            if (parameterInfo.HasDefaultValue && flags.HasFlag(ApiEntryStringFlags.DefaultValues))
            {
                if (parameterInfo.ParameterType.IsEnum) return $"{string.Join(" ", segments)} = {parameterInfo.ParameterType.Name}.{parameterInfo.DefaultValue}";
